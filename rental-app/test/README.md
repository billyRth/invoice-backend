...
`pteas.html` itself is never modified by the test.

```sh
# 1. a Postgres on /tmp:5433, then:
psql -d postgres -c 'create database pteas_app'
psql -d pteas_app -f supabase/tests/00-local-shim.sql
for m in 0001_init 0002_payments 0004_lock_down_functions 0005_districts; do
  psql -d pteas_app -f "supabase/migrations/$m.sql"
done
psql -d pteas_app -f supabase/seed.sql
...
The shim implements only the calls `pteas.html` makes. It is a fixture, not a
PostgREST clone.
